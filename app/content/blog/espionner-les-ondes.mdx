...
Nous vivons dans cet océan d'ondes électromagnétiques. Et comme vous allez le découvrir, beaucoup des appareils qui nous entourent sont étonnamment bavards...
...
L'image ci-dessus est le résultat de quelques minutes d'attentes à Paris. A l'heure où j'ai « écouté » les avions, il y en avait 2 dans le ciel que j'ai pu suivre un peu. Pour chaque point sur cette carte, je peux connaître :
...
<TechnicalImage
  src="/assets/blog/sdr/wifi-packets.webp"
  alt="Paquets WiFi interceptés avec Scapy"
  caption="Analyse de trafic WiFi révélant les métadonnées des communications (données sensibles masquées)"
/>

Bien que le contenu des communications soit généralement chiffré, les métadonnées de ces communications restent visibles. Comme on peut le voir sur cette capture, il est possible d'observer :

- L'identité des appareils connectés
- Les volumes et schémas de trafic
- Les tentatives de connexion

Les appareils mobiles sont particulièrement bavards : ils émettent constamment des "probe requests" cherchant les réseaux auxquels ils se sont déjà connectés, révélant ainsi votre historique de connexions (domicile, travail, hôtels visités...). Cette transparence du trafic permet à un observateur passif de déterminer facilement les habitudes des utilisateurs : périodes d'activité intense le matin et le soir, ou au contraire, absence prolongée du domicile.
