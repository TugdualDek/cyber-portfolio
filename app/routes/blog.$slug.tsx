...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  return [
    ...
    ...(data.meta.coverImage
      ? [{ property: "og:image", content: data.meta.coverImage }]
      : []),
  ];
};
...
const MDXComponents = {
  h1: (props: any) => (
    <h1
      className="text-2xl text-cyber-primary font-bold mb-4 mt-8"
      {...props}
    />
  ),
  h2: (props: any) => (
    <h2 className="text-xl text-cyber-primary font-bold mb-3 mt-6" {...props} />
  ),
  h3: (props: any) => (
    <h3
      className="text-lg text-cyber-primary font-semibold mb-3 mt-5"
      {...props}
    />
  ),
  p: (props: any) => (
    <p className="mb-4 leading-relaxed text-white/90" {...props} />
  ),
  ul: (props: any) => (
    <ul className="list-disc pl-5 mb-4 text-white/90 space-y-1" {...props} />
  ),
  ol: (props: any) => (
    <ol className="list-decimal pl-5 mb-4 text-white/90 space-y-1" {...props} />
  ),
  ...
  blockquote: (props: any) => (
    <blockquote
      className="border-l-4 border-yellow-500/40 pl-4 py-1 my-4 bg-black/20"
      {...props}
    />
  ),
  ...
  code: memo((props: any) => {
    ...
    return (
      <code
        className="bg-black/40 text-cyber-primary px-1.5 py-0.5 rounded text-sm"
        {...props}
      />
    );
  }),
  ...
  th: (props: any) => (
    <th
      className="bg-yellow-900/30 text-cyber-primary px-4 py-2 border border-yellow-500/30 text-left"
      {...props}
    />
  ),
  td: (props: any) => (
    <td
      className="px-4 py-2 border border-yellow-500/20 text-white/90"
      {...props}
    />
  ),
  hr: (props: any) => (
...
const BackButton = memo(() => (
  <Link
    to="/blog"
    className="inline-flex items-center mb-6 px-3 py-1.5 bg-black/40 border border-yellow-500/40
            hover:bg-yellow-900/20 hover:border-yellow-500/60 transition-all duration-200 rounded-xl"
    prefetch="intent"
  >
    <svg
...
const DocumentHeader = memo(
  ({
    ...
    formattedExecutionDate: string | null;
    classStyles: any;
  }) => (
    <>
      {/* Bannière modifiée pour être plus subtile */}
      <div
        className={`w-full py-1.5 text-center font-mono font-bold tracking-[0.25em] text-sm ${classStyles.banner} rounded-t-xl`}
      >
        {meta.classification}
      </div>
      ...
      <div className="relative">
        ...
        <div className="p-8 relative">
          ...
          <div className="mt-12 mb-8">
            <h1 className="text-center text-3xl font-bold text-cyber-primary mb-2">
              {meta.title}
            </h1>
            <div className="w-1/3 mx-auto border-b-2 border-yellow-500/40"></div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4 max-w-2xl mx-auto text-sm text-center my-6">
            <div>
              <span className="text-cyber-primary/70">CASE NUMBER:</span>
              <span className="ml-2 text-white/90">
                #{meta.caseNumber || meta.date.replace(/-/g, "")}
              </span>
            </div>
            <div>
          ...
          <div className="absolute top-20 right-8 hidden md:block">
            <div
              className={`transform rotate-12 border-2 ${classStyles.stamp} px-4 py-2 text-xs font-mono font-bold opacity-80`}
            >
              {meta.classification}
            </div>
          </div>
...
const DocumentFooter = memo(
  ({ meta, classStyles }: { meta: PostMeta; classStyles: any }) => (
    <div className="p-6 border-t border-yellow-500/20 flex justify-between items-center text-xs bg-black/30 rounded-b-xl">
      <div className="text-cyber-primary/70">
        DOCUMENT ID: TDK-{meta.date.substring(0, 4)}-
        {meta.caseNumber || meta.slug}
      </div>
      <div className="flex items-center space-x-3">
        <div className={`${classStyles.border} border px-2 py-0.5`}>
          {meta.classification}
        </div>
        <div className="text-cyber-primary/70">Page 1 of 1</div>
      </div>
    </div>
  )
);
...
export default function BlogPost() {
  ...
  const classStyles = useMemo(() => {
    const key =
      meta.classification.toUpperCase() as keyof typeof CLASSIFICATION_STYLES;
  ...
  return (
    <div
      key={`blog-post-${meta.slug}`}
      className="min-h-screen text-cyber-primary"
    >
      <div className="relative z-10">
        ...
        <main className="container mx-auto px-4 md:px-6 pt-8 pb-16">
          ...
          <div className="max-w-6xl mx-auto bg-navy-950/95 border border-yellow-500/30 backdrop-blur-md shadow-xl rounded-xl">
            <DocumentHeader
              meta={meta}
            ...
            {/* Ajouter un espace après le résumé */}
